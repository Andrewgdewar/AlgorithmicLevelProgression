...
const modName = `${author.replace(/[^a-z0-9]/gi, "")}-${packageName.replace(
  /[^a-z0-9]/gi,
  ""
)}-${version}`;
const modNameWithoutVersion = `${author.replace(
  /[^a-z0-9]/gi,
  ""
...
fs.copySync(
  __dirname,
  path.normalize(`${__dirname}/../~${modNameWithoutVersion}`),
  {
    filter: (filePath) => {
...
zip({
  ...
  .then(function () {
    ...
    fs.rmSync(`${__dirname}/${modNameWithoutVersion}`, {
      force: true,
      recursive: true,
    });
    console.log(
