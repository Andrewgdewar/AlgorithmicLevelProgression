...
export default function RecoilAndAmmoChanges(container: DependencyContainer): undefined {
    if (config?.enableRecoilAndAmmoChanges) {
        ...
        Object.keys(tables?.templates?.items || {}).forEach(name => {
            ...
            if (bulletType?.PenetrationPower && bulletType.Name !== "Shrapnel") {
                ...
                adjustableValueList.forEach(({ name, callback }) => {
                    if (!!config.AmmoChanges[name] && bulletType[name] !== undefined) {
                        const oldValue = bulletType[name]
                        bulletType[name] = callback(bulletType[name], config.AmmoChanges[name])
