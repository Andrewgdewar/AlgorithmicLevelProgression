...
const deleteBlacklistedItemsFromInventory = (inventory) => {
    Object.keys(inventory.items).forEach((key) => {
        inventory.items[key] = inventory.items[key].filter((id) => !exports.blacklistedItems.has(id));
    });
...
const ensureAllAmmoInSecureContainer = (inventory) => {
    const ammo = Object.keys(inventory.Ammo)
        .map((calbr) => Object.keys(inventory.Ammo[calbr]))
        .flat();
    const securContainerListSet = new Set(inventory?.items?.SecuredContainer || []);
    ammo.forEach((id) => {
...
exports.ensureAllAmmoInSecureContainer = ensureAllAmmoInSecureContainer;
const addAllSecureContainers = (inventory) => {
...
exports.addAllSecureContainers = addAllSecureContainers;
