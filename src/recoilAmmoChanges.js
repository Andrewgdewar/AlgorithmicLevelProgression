...
function RecoilAndAmmoChanges(container) {
    if (config_json_1.default?.enableRecoilAndAmmoChanges) {
        ...
        Object.keys(tables?.templates?.items || {}).forEach(name => {
            ...
            if (bulletType?.PenetrationPower && bulletType.Name !== "Shrapnel") {
                ...
                adjustableValueList.forEach(({ name, callback }) => {
                    if (!!config_json_1.default.AmmoChanges[name] && bulletType[name] !== undefined) {
                        const oldValue = bulletType[name];
                        bulletType[name] = callback(bulletType[name], config_json_1.default.AmmoChanges[name]);
...
exports.default = RecoilAndAmmoChanges;
