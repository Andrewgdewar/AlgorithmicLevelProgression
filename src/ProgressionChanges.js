...
function ProgressionChanges(container) {
    ...
    for (let index = 0; index < numList.length; index++) {
        ...
        const copyOfPreviousWhitelist = !!updatedData.whitelist[index - 1] ?
            (0, utils_1.cloneDeep)(updatedData.whitelist[index - 1]) :
            { equipment: {}, cartridge: {} };
        const whitelistItem = {
        ...
        for (let k = 0; k < itemList.length; k++) {
            ...
            if (parent === AmmoParent) {
                ...
                if (!weightingAdjustmentItem.ammo.add?.[calibre]) {
                    weightingAdjustmentItem.ammo.add = { ...weightingAdjustmentItem.ammo.add, [calibre]: {} };
                }
                const ammoWeight = (0, utils_1.getAmmoWeighting)(items[id]);
        ...
        for (const key of updatedData.whitelist) {
            (0, utils_1.mergeDeep)(combinedWhiteLists, key);
        }
        const combinedWeightingAdjustmentItem = {};
        for (const key of updatedData.weightingAdjustments) {
            (0, utils_1.mergeDeep)(combinedWeightingAdjustmentItem, key);
        }
        (0, utils_1.mergeDeep)(combinedWeightingAdjustmentItem, weightingAdjustmentItem);
        for (let k = 0; k < itemList.length; k++) {
            const id = itemList[k];
            const item = items[id];
            const parent = item._parent;
            ...
            switch (true) {
                case items[parent]?._parent === "5422acb9af1c889c16000029": // Ammo Parent
                    ...
                    if (combinedWhiteLists?.cartridge?.[calibre]) {
                        ...
                        const highestScoringAmmo = (0, utils_1.getHighestScoringAmmoValue)(combinedWeightingAdjustmentItem.ammo.add[calibre]);
                        const weaponRating = (0, utils_1.getWeaponWeighting)(item, highestScoringAmmo);
                ...
                case parent === "5448e54d4bdc2dcc718b4568": //"ArmorParent"
                    ...
                    const armorRating = (0, utils_1.getArmorRating)(item);
